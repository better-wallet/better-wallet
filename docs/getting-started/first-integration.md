...
Sign an Ethereum transaction (without broadcasting):

```bash
curl -X POST "$BW_API/v1/wallets/$WALLET_ID/sign" \
  -H "X-App-Id: $APP_ID" \
  -H "X-App-Secret: $APP_SECRET" \
  -H "Authorization: Bearer $JWT_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "$BW_API/v1/wallets/$WALLET_ID/sign-message" \
  -H "X-App-Id: $APP_ID" \
  -H "X-App-Secret: $APP_SECRET" \
  -H "Authorization: Bearer $JWT_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "$BW_API/v1/wallets/$WALLET_ID/sign-typed-data" \
  -H "X-App-Id: $APP_ID" \
  -H "X-App-Secret: $APP_SECRET" \
  -H "Authorization: Bearer $JWT_TOKEN" \
  -H "Content-Type: application/json" \
...
async function signTransaction(walletId, jwtToken, txParams) {
  const response = await fetch(
    `${BETTER_WALLET_API}/v1/wallets/${walletId}/sign`,
    {
      method: 'POST',
      headers: {
        'X-App-Id': APP_ID,
...
async function main() {
  ...
  console.log(`Signed tx: ${signed.signed_transaction}`);
}
```
